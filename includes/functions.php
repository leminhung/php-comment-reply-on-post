...
// connect to database
$mysqli = new mysqli("localhost", "root", "12345678", "comment-reply-system");

// get all posts from database
$post_query_result = $mysqli->query("SELECT * FROM posts WHERE id=1");
$post = mysqli_fetch_assoc($post_query_result);

// Get all comments from database
$comments_query_result = $mysqli->query("SELECT * FROM comments WHERE post_id=" . $post['id'] . " ORDER BY created_at DESC");
...
if (isset($_POST['comment_posted'])) {
	...
	$res = $mysqli->query("SELECT * FROM comments WHERE id=$inserted_id");
...
if (isset($_POST['reply_posted'])) {
	...
	$inserted_id = $mysqli->insert_id;
	$res = $mysqli->query("SELECT * FROM replies WHERE id=$inserted_id");
...
{
	global $mysqli;
	$result = $mysqli->query("SELECT username FROM users WHERE id=" . $id . " LIMIT 1");
